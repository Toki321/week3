...
// Number Mastermind (6 digits, 4 holes, sum as hint) 
template MastermindVariation() {
    ...
    for (j=0; j<4; j++) {
        lessThan[j] = LessThan(4);          //gets component, 4 is num of bits
        lessThan[j].in[0] <== guess[j];     //asserts and assigns  that input1 is a member of the array guess with index j
        lessThan[j].in[1] <== 6;           //asserts and assigns that input2 is 6 bcs we have 0-5
        lessThan[j].out === 1;              //asseerts that in[0] is less than 6
        lessThan[j+4] = LessThan(4);        //+4 means its going to be for the solution array
        lessThan[j+4].in[0] <== soln[j];    // input 1 is soln[j]
        lessThan[j+4].in[1] <== 6;         // input2 is 6
        lessThan[j+4].out === 1;            // assert soln[j] < 6 
        for (k=j+1; k<4; k++) {
    ...
    // Sum of the priv inputs as a hint
