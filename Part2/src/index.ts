...
const encrypt = async (
  ...
  const iv = buf2Bigint(mimc7.multiHash(plaintext, BigInt(0)));
   ...
   // console.log("iv: ", iv);
    ...
    const big_int =  plaintext.map((e: bigint, i: number): bigint => {
        return e + buf2Bigint(mimc7.hash(sharedKey, iv + BigInt(i)))
    });


    const enPlTxt: Ciphertext = {
        iv,
        data: big_int,
    }
...
const decrypt = async (
  ...
  const dePlTxt: Plaintext = ciphertext.data.map((e: bigint, i: number): bigint => {
      return BigInt(e) - buf2Bigint(MIMC.hash(sharedKey, BigInt(ciphertext.iv) + BigInt(i))) 
  });
